...
const SYSTEM_PROMPT = "你叫灵犀，是一位经验丰富的塔罗占卜师。解读风格温暖有洞察力，有灵性不油腻，给人希望。";
...
export async function getTarotReading(data: ReadingInput) {
  ...
  try {
    res = await fetch("https://api.deepseek.com/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: "deepseek-chat", messages: [{ role: "system", content: SYSTEM_PROMPT }, { role: "user", content: userPrompt }], temperature: 0.85, max_tokens: data.tier === "relationship" ? 3500 : data.tier === "deep" ? 2800 : 1400 }),
    });
  } catch { throw new Error("宇宙信号暂时中断，请稍后再试 ✨"); }
  if (!res.ok) throw new Error("宇宙信号暂时中断，请稍后再试 ✨");
